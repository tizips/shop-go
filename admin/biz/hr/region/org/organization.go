...
import (
	...
	"gorm.io/gorm"
	req "project.io/shop/admin/http/request/hr/region/org"
	res "project.io/shop/admin/http/response/hr/region/org"
	"project.io/shop/model"
)

func ToOrganizationOfPaginate(c context.Context, ctx *app.RequestContext) {

	var request req.ToOrganizationByPaginate
...
func DoOrganizationOfCreate(c context.Context, ctx *app.RequestContext) {
	...
	if request.Parent == "" {
		...
		var parent []model.HRChildren

		tx.Order("`id` asc").Find(&parent, "`child_id`=?", organization.ParentID)
...
func DoOrganizationOfEnable(c context.Context, ctx *app.RequestContext) {
	...
	if uo := facades.Gorm.Model(&organization).Update("is_enable", request.IsEnable); uo.Error != nil {
		http.Fail(ctx, "修改失败")
		return
	}
